...
All handling of collision detection and solving.

### AABB

Axis Aligned Bounding Box, used for broad phase collision detection.

[Vector2](#vector2) min is the minimum value for the box (the bottom left corner).
[Vector2](#vector2) max is the maximum value for the box (the top right corner).

#### Overlaps

Returns true if the AABB overlaps with another AABB.

### Contact

[Contact.hpp](/Collision/Contact.hpp)

Struct for a contact point of a collision.

[Vector2](#vector2) point is the position of the contact. normalImpulse, frictionImpulse are used as accumulators for the impulses during collision solving iterations. targetVelocity is used for applying the correct restitution through iterations.

### CollisionPair

Struct holding references to a pair of [Rigidbodies](#rigidbody).

### Manifold

Collision manifold, generated by the [CollisionDetector](#collisiondetector), used by the [CollisionSolver](#collisionsolver).

## CollisionDetector

[CollisionDetector.hpp](/Collision/CollisionDetector.hpp)
[CollisionDetector.cpp](/Collision/CollisionDetector.cpp)

Static class for collision detection.

### Projection

Struct to hold a projection of a shape onto an axis.

### ProjectPolygon

Returns a [Projection](#projection) of a polygon onto an axis.
...
Returns true if the [Projections](#projection) overlap.

### GetOverlap

Returns the amount of overlap between [Projections](#projection).

### GetAABB

Returns the [AABB](#aabb) of a shape.

### ClipSegment

Clips the 'in' [Vector2s](#vector2) to the offset on the normal.

### BroadPhase

Finds pairs of potentially colliding bodies using [AABB](#aabb) overlaps.

### NarrowPhase

Detects collisions for [CollisionPairs](#collisionpair), and returns the [manifolds](#manifold). Invokes [DetectCollision](#detectcollision).

### DetectCollision

Takes a pair of [Rigidbodies](#rigidbody). Determines the shapes of the bodies and calls one of: [CircleVCircle](#circlevcircle), [PolyVPoly](#polyvpoly), [PolyVCircle](#polyvcircle).

### CircleVCircle

Generates a [Manifold](#manifold) for collisions between [Circles](#circle). Checks positions against radii.

### PolyVPoly

Uses SAT to detect collisions between a pair of [Polygons](#polygon). Generates the [Manifold](#manifold) for the collision.

### PolyVCircle

Uses SAT to detect collisions between a [Polygon](#polygon) and a [Circle](#circle). [Projects](#projection) both shapes onto axis of edges on the [Polygon](#polygon). Generates the [Manifold](#manifold) for the collision, finds the contact point as a point on the edge of the circle.

## CollisionSolver

[CollisionSolver.hpp](/Collision/CollisionSolver.hpp)
[CollisionSolver.cpp](/Collision/CollisionSolver.cpp)

Static class for solving collisions from a [Manifold](#manifold).

### CollisionSolver Properties

| Property | Description |
| -------- | ----------- |
| slop | The minimum position correction to be made, allowing smaller corrections can lead to jittering |
| correctionPercent | The amount of penetration to correct |
| impulseSlop | The minimum impulse that can be applied, smaller impulses can lead to jittering |
| iterations | The number of iterations used to find correct impulses |

### CorrectPosition

Moves bodies proportional to penetration.

### FindTargetVelocities

Determines the correct velocities after the collision, along the normal at each [contact](#contact).

### SolveNormal

Calculates and applies an impulse along the normal at the [contact](#contact).

### SolveTangent

Calculates and applies an impulse along the tangent at the [contact](#contact), applying a "friction" force.

### SolveCollision

Corrects positions, then uses iterative impulse solving for [SolveNormal](#solvenormal) and [SolveTangent](#solvetangent).